// Vendored from React Bits (https://reactbits.dev), MIT + Commons Clause license.
// Source: github.com/DavidHDev/react-bits, src/ts-default/Animations/SplashCursor.
// Adapted here to bound the fluid sim + pointer input to its own mounted
// container instead of the full viewport, so it can be scoped to one section
// of a page rather than tracking the cursor everywhere.
'use client';
...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    // Bound the effect to the canvas's own box instead of the full viewport,
    // so it can live inside one section of a page rather than tracking the
    // cursor everywhere. Returns null when the point falls outside it.
    function localPos(clientX: number, clientY: number): { x: number; y: number } | null {
    ...
    window.addEventListener('mousedown', e => {
      const local = localPos(e.clientX, e.clientY);
      if (!local) return;
      const pointer = pointers[0];
      const posX = scaleByPixelRatio(local.x);
      const posY = scaleByPixelRatio(local.y);
    ...
    function handleFirstTouchStart(e: TouchEvent) {
      ...
      for (let i = 0; i < touches.length; i++) {
        const local = localPos(touches[i].clientX, touches[i].clientY);
        if (!local) continue;
        const posX = scaleByPixelRatio(local.x);
        const posY = scaleByPixelRatio(local.y);
        updateFrame();
