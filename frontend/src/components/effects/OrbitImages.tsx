// Component created by Dominik Koch (https://x.com/dominikkoch), via React Bits
// (https://reactbits.dev), MIT + Commons Clause license.
// Source: github.com/DavidHDev/react-bits, src/ts-default/Animations/OrbitImages.
//
// Adapted here to add `links`/`labels`, wrapping each orbiting image in a
// real link to a resource instead of rendering a purely decorative <img>,
// and dropping aria-hidden accordingly since the content is now navigable.
...
interface OrbitImagesProps {
  images?: string[];
  /** Optional, parallel to images: where each image links to when clicked. */
  links?: string[];
  /** Optional, parallel to images: alt text / accessible label for each image. */
  labels?: string[];
  altPrefix?: string;
  shape?: OrbitShape;
  customPath?: string;
...
export default function OrbitImages({
  ...
  const items = images.map((src, index) => {
    const label = labels[index] ?? `${altPrefix} ${index + 1}`;
    const img = <img src={src} alt={label} draggable={false} className="orbit-image" />;
    const href = links[index];
    return href ? (
      <a key={src} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} className="orbit-image-link">
        {img}
      </a>
    ) : (
      <span key={src}>{img}</span>
    );
  });
