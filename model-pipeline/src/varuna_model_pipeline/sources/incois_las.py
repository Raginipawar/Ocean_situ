"""INCOIS LAS ingestion for gridded ocean model datasets.

This module is intentionally source-specific. It accepts an xarray-readable
NetCDF/local dataset path or URL, inspects its coordinates and variables, maps
known INCOIS LAS-style names into the canonical VARUNA model schema, and returns
a standardized xarray Dataset.

It does not perform Bay of Bengal extraction, unit conversion, or regridding;
...
from ..errors import MissingCoordinateError, MissingVariableError, UnsupportedUnitError
...
COORDINATE_CANDIDATES: dict[str, tuple[str, ...]] = {
    ...
    "depth": ("depth", "depth_m", "DEPTH", "Depth", "lev", "level"),
}

# Explicit default mapping for the INCOIS LAS/demo NetCDF shapes this component
# supports. Real-source differences should be added here or supplied through
# SourceConfig.variable_mapping after inspecting source metadata.
INCOIS_LAS_DEFAULT_MAPPING: dict[str, str] = {
    "sst": "sst",
    "SST": "sst",
    "temperature": "sst",
    "temp": "sst",
...
class IncoisLasIngestionResult:
    """Standardized result returned by the INCOIS LAS ingestor."""

    dataset: xr.Dataset
    source: ModelSource = ModelSource.INCOIS_LAS
...
class IncoisLasIngestor:
    """Load and standardize INCOIS LAS NetCDF/gridded datasets."""
    ...
    def __init__(
        ...
        configured_mapping = variable_mapping or {}
        self.variable_mapping = {
            **INCOIS_LAS_DEFAULT_MAPPING,
            **dict(configured_mapping),
        }

    def load(self, path_or_url: str | Path) -> IncoisLasIngestionResult:
        """Open and standardize an INCOIS LAS xarray-readable dataset."""

        source_ref = str(path_or_url)
        logger.info("Loading INCOIS LAS model dataset from %s", source_ref)
        try:
            raw = xr.open_dataset(path_or_url)
        except Exception as exc:  # noqa: BLE001 - wrap with source context
            from ..errors import IngestionError

            raise IngestionError(f"Could not open INCOIS LAS dataset {source_ref!r}: {exc}") from exc
    ...
    def standardize(
        ...
        *,
        ...
        """Return a canonical Dataset from an already-open source Dataset."""
        ...
        canonical = xr.Dataset(
            ...
            attrs={
                **renamed.attrs,
                "source": ModelSource.INCOIS_LAS.value,
                "source_url": source_path,
                "region": self.settings.region.name,
                "processing_level": "incois_las_ingested",
        ...
        if not report.ok:
            raise MissingVariableError(
                "Standardized INCOIS LAS dataset failed canonical validation: "
                + "; ".join(report.errors)
            )

        return IncoisLasIngestionResult(
            dataset=canonical,
        ...
        for canonical_name in ("time", "lat", "lon"):
            ...
            if not source_name:
                raise MissingCoordinateError(
                    f"INCOIS LAS dataset is missing required coordinate {canonical_name!r}. "
    ...
    def _resolve_variables(self, dataset: xr.Dataset) -> dict[str, str]:
        ...
        for source_var, data_array in dataset.data_vars.items():
            ...
            if canonical_name not in CANONICAL_VARIABLES:
                raise MissingVariableError(
                    f"INCOIS mapping for source variable {source_var!r} points to "
                    f"unknown canonical variable {canonical_name!r}"
                )
            if canonical_name in resolved_by_canonical:
                raise MissingVariableError(
                    f"Multiple INCOIS variables map to {canonical_name!r}: "
        ...
        if missing:
            raise MissingVariableError(
                f"INCOIS LAS dataset missing required canonical variable(s): {missing}. "
    ...
    def _validate_units(self, canonical_name: str, data_array: xr.DataArray) -> None:
        ...
        if units is None:
            raise UnsupportedUnitError(
                f"INCOIS variable {data_array.name!r} mapped to {canonical_name!r} "
    ...
    def _append_history(existing_history: object) -> str:
        ...
        entry = "VARUNA model-pipeline: standardized INCOIS LAS source variables."
